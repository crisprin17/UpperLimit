...
void Upper_limit_first(){
  ...
  //Background and Signal systematic uncertainties, to be calculated based on systematic studies                                                                                                 
  double Bkg_st = 0.15;
  double Sig_st = 0.15;

  const int sig_evt_size = 30;
...
//OPTIONAL: plot the background and background plus signal distributions
  TH1D** arrayN = new TH1D*[sig_evt_size]; for (int k=0;k<sig_evt_size;k++) { arrayN[k] = new TH1D(Form("b%d",k),"NullHypot; Number of Events; Entries;",200,0,125); }
  TH1D** array  = new TH1D*[sig_evt_size]; for (int y=0;y<sig_evt_size;y++) { array[y] = new TH1D(Form("h%d",y),"TestHypot; Number of Events; Entries;",200,0,125); }
...
//We don't know the number of signal events to expect so we will loop over a range of possible number of events m that goes from 5 to 30  
 for(int m = sig_evt_min; m < (sig_evt_size); m++){
    ...
    //plot a line at the expected limit
    ...
    legend->AddEntry(line,"Expected Limit", "l");
